...
//
// [问题描述]
// 在 “Dick Van Dyke“ 的某一集中，小 Richie 尝试把他爸爸背上的斑点连成独立钟的图案。但是其中
// 一个斑点是伤疤，所以 Richie 的尝试失败了。
//
// 把 Dick 的背看成一个平面，上面有不同位置的斑点 （x，y）。你的任务是要告诉 Richie 应该怎样连
// 接所有点，使得所用的墨水最少。Richie 总是用直线段连接两个点，在画不同线段之间可以将笔提起。当
// Richie 结束画线时，任何一个斑点必须能够经过所画的线到达任何另外一个斑点。
//
// [输入]
// 输入第一行只有一个正整数，代表测试数据的数量。接下来有一个空行。
//
// 每组数据的第一行有一个整数 n（0 < n <= 100），表示 Dick 背上的斑点数。接下来的每行有两个实
// 数 （x，y），分别代表他背上每个斑点的位置。
//
// 相邻两组输入数据之间有一个空行。
//
// [输出]
// 对于每组数据，你的程序要输出一个保留两位小数的实数，表示将所有斑点连接起来所需线段的最小总长度。
//
// 相邻两组数据的输出之间应该有一个空行。
//
// [样例输入]
