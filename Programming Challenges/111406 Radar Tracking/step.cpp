...
void possibleSolutions(point first, point second)
{
	...
	else
	{
			...
			(first.angle < second.angle && angleDiff > 180.0))
		{
			...
			if (first.angle < second.angle && angleDiff > 180.0)
				C = first.angle + (360.0 - second.angle);
			else
				C = first.angle - second.angle;
            ...
            double x = 0, minX = 0;
            double lastValue = -1;
            for (double i = (180.0 - degrees(B) - originalC); i >= 0.01; i -= 0.01)
            {
                x = radians(i);
                double value = b * sin(x) - sin(B + C + x) * c * (2 * PI - x) / (2 * PI - C);
                if (lastValue < 0)
                    lastValue = fabs(value);
                else if (fabs(value) < lastValue)
                {
                    lastValue = fabs(value);
                    minX = x;
                }
			...
			finalAngle = second.angle - degrees(minX);
			if (finalAngle < 0.0)
				finalAngle += 360.0;
			finalDistance = b * sin(B + C) / sin(B + C + minX);
