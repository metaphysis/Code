...
int get_min_weight_diff()
{
    // 标记已经使用的边。
    int *used = new int[number_of_edges];
...
int main(int argc, char *argv[])
{
    ...
    for (int c = 1; c <= cases; c++) {
        ...
        int min_weight_sum = prim(0);
        int min_weight_diff = get_min_weight_diff();

        cout << min_weight_sum << ' ' << (min_weight_sum + min_weight_diff) << '\n';
    }
