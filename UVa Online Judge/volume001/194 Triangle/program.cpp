...
bool calculate()
{
    ...
    if (defined['A'] && defined['B'] && defined['C'])
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            if (defined[sideAngles[i][0]] &&
                defined[sideAngles[i][1]] && defined[sideAngles[i][2]])
            {
                double temp = paras[sideAngles[i][0]] /
                    sin(paras[sideAngles[i][1]]) * sin(paras[sideAngles[i][2]]);
                if (defined[sideAngles[i][4]] &&
                    fabs(temp - paras[sideAngles[i][4]]) >= EPSILON)
                    return false;
                else
                    paras[sideAngles[i][4]] = temp;

                temp = paras[sideAngles[i][0]] /
                    sin(paras[sideAngles[i][1]]) * sin(paras[sideAngles[i][3]]);
                if (defined[sideAngles[i][5]] &&
                    fabs(temp - paras[sideAngles[i][5]]) >= EPSILON)
                    return false;
                else
    ...
    for (int i = 0; i < 3; i++)
    {
        if (defined[cosines[i][0]] && defined[cosines[i][1]]
            && defined[cosines[i][2]])
        {
            double d[6];
            ...
            // Law of Cosines
            double temp =
                sqrt(d[0] * d[0] + d[1] * d[1] - 2 * d[0] * d[1] * cos(d[2]));
            ...
            // Law fo Sines
            temp =
                asin(paras[cosines[i][0]] / paras[cosines[i][3]] *
                sin(paras[cosines[i][2]]));
            ...
            // Law fo Sines
            temp =
                asin(paras[cosines[i][1]] / paras[cosines[i][3]] *
                sin(paras[cosines[i][2]]));
    ...
    string sines[] =
        { "abABCc", "baBACc", "acACBb", "caCABb", "bcBCAa", "cbCBAa" };
    for (int i = 0; i < 6; i++)
    {
        double d[6];
        ...
        if (defined[sines[i][0]] && defined[sines[i][1]]
            && defined[sines[i][2]])
        {
            // Law fo Sines
            double temp =
                asin(paras[sines[i][1]] / paras[sines[i][0]] *
                sin(paras[sines[i][2]]));
            ...
            // Law fo Sines
            temp =
                paras[sines[i][0]] / sin(paras[sines[i][2]]) *
                sin(paras[sines[i][4]]);
