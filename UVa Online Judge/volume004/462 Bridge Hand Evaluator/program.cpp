...
int main(i) {
    ...
    while (getline(cin, line)) {
        ...
        for (int i = 0; i < 4; i++) {
            if (countOfFaces[i][1] > 0 && countOfFaces[i][1] == countOfSuits[i]) points -= countOfFaces[i][1];
            if (countOfFaces[i][2] > 0 && countOfFaces[i][2] >= countOfSuits[i] - 1) points -= countOfFaces[i][2];
            if (countOfFaces[i][3] > 0 && countOfFaces[i][3] >= countOfSuits[i] - 2) points -= countOfFaces[i][3];
        }
