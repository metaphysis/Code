...
bool is_too_big(string number, long long max_value) {
    if (number.length() > 10 || (number.length() == 10 && number.front() > '2')) return true;
    else {
...
int main() {
    ...
    while (getline(cin, line)) {
        ...
        if (middle == "*") {
            ...
            if (first.length() + second.length() >= 12) cout << "result too big\n";
            else {
                long long first_value = stoll(first), second_value = stoll(second);
                long long result_value = first_value * second_value;
                if (result_value > numeric_limits<int>::max()) cout << "result too big\n";
            }
            continue;
        }
        int sign = 1;
