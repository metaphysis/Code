...
int main() {
    ...
    while (getline(cin, line)) {
        ...
        while (iss >> cycle) cycles.push_back(cycle);
        sort(cycles.begin(), cycles.end());
        priority_queue<instant> times;
        for (int i = 0; i < cycles.size(); i++) times.push((instant){cycles[i] * 2, cycles[i] * 2});
        while (times.empty() == false) {
            instant x = times.top();
            times.pop();
            int allGreen = true;
            for (int i = 0; i < cycles.size(); i++) {
                if ((x.seconds / cycles[i]) % 2 == 0 && (x.seconds % cycles[i]) < (cycles[i] - 5)) continue;
                else {
                    allGreen = false;
                    break;
                }
