...
struct item {
    ...
    bool operator<(const item& another) const {
        if (frequency != another.frequency) return frequency > another.frequency;
        else return letter < another.letter;
    }
};
...
int main() {
    ...
    for (int i = 1; i <= cases; i++) {
        ...
        for (auto p : plain) text1.push_back((item){p.first, p.second});
        for (auto p : encoded) text2.push_back((item){p.first, p.second});
        sort(text1.begin(), text1.end());
        sort(text2.begin(), text2.end());
        map<char, char> code;
        for (int j = 0; j < text1.size(); j++) code[text2[j].letter] = text1[j].letter;
        for (auto c : encoded_line)
            if (isalpha(c)) cout << code[c];
            else cout << c;
        cout << '\n';
    }
    return 0;
