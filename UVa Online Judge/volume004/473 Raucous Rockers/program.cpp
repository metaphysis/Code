...
int dfs(int nSong, int mDisk, int tMinute) {
    ...
    if (tMinute + songs[nSong] < t) r = max(r, 1 + dfs(nSong + 1, mDisk, tMinute + songs[nSong]));
    if (tMinute + songs[nSong] == t) r = max(r, 1 + dfs(nSong + 1, mDisk + 1, 0));
    if (tMinute + songs[nSong] > t) r = max(r, dfs(nSong, mDisk + 1, 0));
