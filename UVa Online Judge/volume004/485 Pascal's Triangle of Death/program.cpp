...
int compare(const BigInteger& x, const BigInteger& y) {
    // 符号不同，正数大于负数。
    if (x.sign == POSITIVE && y.sign == NEGATIVE || x.sign == NEGATIVE && y.sign == POSITIVE) return (x.sign == POSITIVE ? 1 : -1);
...
int main() {
    ...
    BigInteger number[220];
    for (int i = 1; i <= 205; i++) {
        number[i] = BigInteger(1);
        for (int j = i - 1; j > 1; j--) number[j] = number[j] + number[j - 1];
        for (int j = 1; j <= i; j++) {
            if (j > 1) cout << ' ';
            cout << number[j];
        }
