...
ostream& operator<<(ostream& os, const BigInteger& number) {
    ...
    for (int i = number.digits.size() - 2; i >= 0; i--) os << setw(number.width) << setfill('0') << number.digits[i];
    return os;
}
...
int main() {
    ...
    vector<BigInteger> fibonacci(5010);
    fibonacci[0] = ZERO, fibonacci[1] = ONE;
    for (int i = 2; i <= 5000; i++) fibonacci[i] = fibonacci[i - 1] + fibonacci[i - 2];
    int number;
    while (cin >> number) cout << "The Fibonacci number for " << number << " is " << fibonacci[number] << endl;
    return 0;
}
