...
bool matched(string mask, string text) {
    if (mask.back() == '*') {
        if ((mask.length() - 1) > text.length()) return false;
...
bool findWildcard() {
    ...
    for (auto f : deleted) {
        ...
        if (min_name_length > f.name.length()) min_name_length = f.name.length();
        if (max_extension_length < f.extension.length()) {
            mask_extension = f.extension;
            max_extension_length = f.extension.length();
        }
        if (min_extension_length > f.extension.length()) min_extension_length = f.extension.length();
    }
    for (auto f : deleted) {
    ...
    if (min_name_length < max_name_length) mask_name = mask_name.substr(0, min_name_length) + "*";
    if (min_extension_length < max_extension_length) mask_extension = mask_extension.substr(0, min_extension_length) + "*";
    return validate();
}
...
int main() {
    ...
    for (int c = 1; c <= cases; c++) {
        ...
        while (getline(cin, line), line.length() > 0) {
            ...
            if (line.front() == '-') deleted.push_back(file{name, extension});
            else reserved.push_back(file{name, extension});
        }
        if (findWildcard()) cout << "DEL " << mask_name << '.' << mask_extension << '\n';
