...
void rotate(int i, int j, int x, int y, int z, int x0, int y0, int L, int W, int H) {
    if (z == 0) curve = min(curve, x * x + y * y);
    else {
        if ((i >= 0) && (i < 2)) rotate(i + 1, j, x0 + L + z, y, x0 + L - x, x0 + L, y0, H, W, L);
        if ((j >= 0) && (j < 2)) rotate(i, j + 1, x, y0 + W + z, y0 + W - y, x0, y0 + W, L, H, W);
        if ((i <= 0) && (i > -2)) rotate(i - 1, j, x0 - z, y, x - x0, x0 - H, y0, H, W, L);
        if ((j <= 0) && (j > -2)) rotate(i, j - 1, x, y0 - z, y - y0, x0, y0 - H, L, H, W);
    }
}
