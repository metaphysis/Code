...
int indexer = 0;
map<string, bool> explicitly;
map<string, int> installed;
map<int, string> sequence;
...
void parse(string line) {
    ...
    if (action == "DEPEND") {
        ...
        iss >> software;
        if (installed.find(software) != installed.end()) cout << "   " << software << " is already installed.\n";
        else install(software, true);
    } else if (action == "REMOVE")     {
        iss >> software;
        if (installed.find(software) == installed.end()) cout << "   " << software << " is not installed.\n";
        else {
            if (referencedBy[software].size() > 0) cout << "   " << software << " is still needed.\n";
            else remove(software, true);
        }
    } else if (action == "LIST") {
        for (auto s : sequence)
            cout << "   " << s.second << '\n';
