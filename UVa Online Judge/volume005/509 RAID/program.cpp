    ...
    while (cin >> disk, disk > 0) {
        ...
        for (int i = 0, p = 0; i < block; i++, p++, p %= disk) {
            for (int j = 0; j < size; j++) {
                string bits;
                for (int k = 0; k < disk; k++) bits += data[i][k][j];
        ...
        print:
        cout << "Disk set " << ++cases;
        if (valid) cout << " is valid, contents are: " << hexText << '\n';
