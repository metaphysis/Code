    ...
    if (fabs(allMaps[a].area - allMaps[b].area) < epsilon) {
        ...
        if (fabs(dist1 - dist2) < epsilon) {
            ...
            if (fabs(diff1 - diff2) < epsilon) {
                ...
                if (fabs(dist1 - dist2) < epsilon) return allMaps[a].leftx < allMaps[b].leftx;
                else return dist1 < dist2;
            } else return diff1 < diff2;
        } else return dist1 < dist2;
    } else return allMaps[a].area > allMaps[b].area;
}
...
bool readLocation() {
    ...
    if (candidate.size() > 0) {
        ...
        for (int i = 0; i < candidate.size(); i++) {
            if (fabs(allMaps[candidate[i]].area - area) < epsilon) info.push_back(make_pair(level, candidate[i]));
            else {
                level += 1;
...
bool readRequest() {
    ...
    if (locations.find(name) == locations.end()) cout << " unknown location\n";
    else {
        if (cache[name].size() == 0) cout << " no map contains that location\n";
        else {
