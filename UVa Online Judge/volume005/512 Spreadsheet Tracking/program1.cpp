        ...
        for (int k = 1; k <= n; k++) {
            ...
            if (command == "DR" || command == "DC" || command == "IR" || command == "IC") {
                ...
                if (command == "DR" || command == "DC") {
                    for (int i = 1; i <= 50; i++) {
                        if (binary_search(x, x + A, i)) z[i] = i;
                        else z[i] = upper_bound(x, x + A, i) - x;
                    }
                    if (command == "DR") {
                        for (int i = 1; i <= r; i++)
                            for (int j = 1; j <= c; j++)
                    ...
                    for (int i = 1; i <= 50; i++) z[i] = upper_bound(x, x + A, i) - x;
                    if (command == "IR") {
                        for (int i = 1; i <= r; i++)
                            for (int j = 1; j <= c; j++)
                                if (grid[i][j].first > 0 && grid[i][j].second > 0)
                ...
                for (int i = 1; i <= r; i++)
                    for (int j = 1; j <= c; j++) {
                        if (grid[i][j].first == originalR && grid[i][j].second == originalC) grid[i][j] = make_pair(nextR, nextC);
                        else if (grid[i][j].first == nextR && grid[i][j].second == nextC) grid[i][j] = make_pair(originalR, originalC);
                    }
            }
        }
        if (cases > 0) cout << '\n';
