...
int main() {
    ...
    while (cin >> n) {
        ...
        n += 1;
        for (int i = 1; i < n; i++) addEdge(0, i, -days[i]);
        dist[0] = 0;
        ...
        do {
            ...
            for (int i = 0; i < nedges; i++) {
                ...
                if (dist[edges[i].v] > weight) updated++, dist[edges[i].v] = weight;
            }
        }
        while (updated && iterations++ < n);
        if (updated) cout << "impossible\n\n";
        else {
            int minDays = dist[1];
            for (int i = 2; i < n; i++) minDays = min(minDays, dist[i]);
            for (int i = 1; i < n; i++) cout << i << ' ' << (dist[i] - minDays) << '\n';
            cout << '\n';
        }
    }
    return 0;
