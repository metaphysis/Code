...
int main() {
    ...
    for (int c = 1; c <= cases; c++) {
        ...
        int value;
        cities = 0;
        while (iss >> value) cost[1][++cities] = value;
        for (int i = 2; i <= cities; i++) {
            getline(cin, line);
            iss.clear();
            iss.str(line);
            int counter = 0;
            while (iss >> value) cost[i][++counter] = value;
        }
        getline(cin, line);
        iss.clear();
        iss.str(line);
        int counter = 1;
        while (iss >> value) tax[counter++] = value;
        memset(path, 0, sizeof(path));
        for (int k = 1; k <= cities; k++)
            for (int i = 1; i <= cities; i++)
                for (int j = 1; j <= cities; j++)
                    if (cost[i][k] >= 0 && cost[k][j] >= 0 && (cost[i][j] == -1 || cost[i][k] + cost[k][j] + tax[k] < cost[i][j])) {
