...
int main() {
    ...
    for (int c = 1; c <= cases; c++) {
        if (c > 1) cout << '\n';
        cin >> row;
        for (int i = 0; i < row.length(); i++) rows[0][i] = row[i];
        N = row.length(), R = 2 * N;
        for (int i = 1; i < R; i++) {
            cin >> row;
            for (int j = 0; j < row.length(); j++) rows[i][j] = row[j];
        }
        ...
        if (exist) {
            ...
            for (int i = 0; i < N; i++)
                for (int j = 0; j < N; j++) {
                    if (grid[j][i] == '2') grid[j][i] = rows[chosen[2 * i + 1]][j];
                    if (grid[j][i] == '2') grid[j][i] = '0';
                }
            cout << "     ";
            for (int i = 0; i < N - 1; i++) cout << setw(3) << left << (chosen[2 * i + 1] + 1);
            cout << chosen[2 * (N - 1) + 1] + 1 << '\n';
            cout << '\n';
            for (int i = 0; i < N; i++) {
                cout << setw(3) << left << (chosen[2 * i] + 1);
                for (int j = 0; j < N; j++) cout << setw(3) << right << grid[i][j];
                cout << '\n';
            }
