...
void displayPath(int i, int j) {
    if (dp[i][j].operation >= DELETE && dp[i][j].operation <= CHANGE) {
        ...
        if (dp[i][j].operation == CHANGE) cout << j << "," << T[j] << "\n";
...
void findPath(int i, int j) {
    if (dp[i][j].operation != NONE) {
        if (dp[i][j].operation == DELETE) findPath(i - 1, j);
        else if (dp[i][j].operation == INSERT) findPath(i, j - 1);
        else findPath(i - 1, j - 1);
    }
    displayPath(i, j);
}
...
void minimumEditDistance() {
    ...
    N = T.length() - 1;
    dp[0][0] = (cell){0, NONE};
    for (int i = 1; i <= M; i++) dp[i][0] = (cell){i, DELETE};
    for (int j = 1; j <= N; j++) dp[0][j] = (cell){j, INSERT};
    // 自底向上动态规划求解。
    for (int i = 1; i <= M; i++)
        for (int j = 1; j <= N; j++) {
            dp[i][j] = (cell){dp[i - 1][j].cost + 1, DELETE};
            if (dp[i][j].cost > (dp[i][j - 1].cost + 1)) dp[i][j] = (cell){dp[i][j - 1].cost + 1, INSERT};
            if (S[i] == T[j]) {
                if (dp[i][j].cost > dp[i - 1][j - 1].cost) dp[i][j] = (cell){dp[i - 1][j - 1].cost, MATCH};
            } else {
                if (dp[i][j].cost > (dp[i - 1][j - 1].cost + 1)) dp[i][j] = (cell){dp[i - 1][j - 1].cost + 1, CHANGE};
            }
        }
    // 反向构建操作步骤。
    deletions = 0;
...
int main() {
    ...
    while (getline(cin, S) && getline(cin, T)) {
        if (printBlankLine == false) printBlankLine = true;
        else cout << "\n";
        minimumEditDistance();
    }
    return 0;
