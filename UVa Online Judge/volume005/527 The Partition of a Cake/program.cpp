...
polygon grahamConvexHull(point vertices[], int number) {
    ...
    pg.number = 0;
    if (number < 3) {
        for (int i = 0; i < number; i++) pg.vertices[i] = vertices[i];
        pg.number = number;
        return pg;
    }
    ...
    int i = 2, top = 1;
    while (i <= number) {
        if (cw(pg.vertices[top - 1], pg.vertices[top], vertices[i])) top--;
        else if (collinear(pg.vertices[top - 1], pg.vertices[top], vertices[i])) pg.vertices[top] = vertices[i++];
        else pg.vertices[++top] = vertices[i++];
    }
    pg.number = top;
    return pg;
}
...
polygon halfPlaneIntersection(line * edges, int n) {
    ...
    for (int i = 2; i < n; i++) {
        if (paralle(deques[top], deques[top - 1]) || paralle(deques[bottom], deques[bottom + 1])) return pg;
        while (bottom < top && cw(edges[i].a, edges[i].b, intersection(deques[top], deques[top - 1]))) top--;
        while (bottom < top && cw(edges[i].a, edges[i].b, intersection(deques[bottom], deques[bottom + 1]))) bottom++;
        deques[++top] = edges[i];
    }
    while (bottom < top && cw(deques[bottom].a, deques[bottom].b, intersection(deques[top], deques[top - 1]))) top--;
    while (bottom < top && cw(deques[top].a, deques[top].b, intersection(deques[bottom], deques[bottom + 1]))) bottom++;
    if (top <= (bottom + 1)) return pg;
    for (int i = bottom; i < top; i++) pg.vertices[pg.number++] = intersection(deques[i], deques[i + 1]);
    if (bottom < (top + 1)) pg.vertices[pg.number++] = intersection(deques[bottom], deques[top]);
    return pg;
}
