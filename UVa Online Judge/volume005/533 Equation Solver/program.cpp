...
const int PLUS = 0, MINUS = 1, MULTIPLY = 2, X = 3, LEFT_BRACE = 4, RIGHT_BRACE = 5, NUMBER = 6;

struct token { int id, coefficient, constant; };
...
vector<token> toPostfix(vector<token> infix) {
    ...
    for (auto t : infix) {
        ...
        if (operators.empty() || operators.top().id == LEFT_BRACE || !lessPriority(t, operators.top())) operators.push(t);
        else {
...
void parse(string expression, int &x, int &constant) {
    ...
    for (int i = 0; i < postfix.size(); i++) {
        if (postfix[i].id == NUMBER || postfix[i].id == X) result.push(postfix[i]);
        else {
            ...
            if (postfix[i].id == PLUS) {
                ...
                if (v1 == 0) result.push(token{NUMBER, v1, v2});
                else result.push(token{X, v1, v2});
...
int main() {
    ...
    while (getline(cin, line)) {
        ...
        cout << "Equation #" << cases << '\n';
        if (left_x == 0) {
            if (right_constant == 0) cout << "Infinitely many solutions.\n";
