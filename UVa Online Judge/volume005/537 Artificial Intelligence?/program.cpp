...
int main() {
    ...
    for (int cases = 1; cases <= n; cases++) {
        ...
        for (auto key : keys) {
            ...
            if (start != line.npos) {
                ...
                for (int i = start; i < line.length(); i++) {
                    if (!isblank(line[i])) block += line[i];
                    else break;
                }
                string number;
                for (int i = 2; i < block.length(); i++)
                    if (isdigit(block[i]) || block[i] == '.' || block[i] == '+' || block[i] == '-') number += block[i];
                    else break;
        ...
        if (values['P'] >= 0 && values['U'] > 0) cout << "I=" << fixed << setprecision(2) << (values['P'] / values['U']) << "A\n\n";
        else if (values['P'] >= 0 && values['I'] > 0)             cout << "U=" << fixed << setprecision(2) << (values['P'] / values['I']) << "V\n\n";
        else cout << "P=" << fixed << setprecision(2) << (values['U'] * values['I']) << "W\n\n";
    }
    return 0;
}
