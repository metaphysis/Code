...
struct term { int coefficient, degree; };
...
int main() {
    ...
    for (int c = 1; c <= cases; c++)     {
        ...
        bool outputed = false;
        for (int i = 10; i >= 0; i--) {
            if (outputed && coefficients[i] > 0) cout << '+';
            if (coefficients[i] > 0) outputed = true;
            if (coefficients[i] > 1) {
                cout << coefficients[i];
                if (i > 0) cout << '*';
            } else {
                if (i == 0 && (!outputed || coefficients[i] == 1)) cout << coefficients[i];
            }
