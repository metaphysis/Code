...
int main() {
    ...
    while (getline(cin, line), line != "END") {
        ...
        while (getline(iss, block, ',')) {
            int number = 0;
            for (int i = 0; i < block.length() && isdigit(block[i]); i++) number = number * 10 + block[i] - '0';
            string direction;
            for (int i = block.length() - 1; i >= 0 && isalpha(block[i]); i--) direction.insert(direction.begin(), block[i]);
