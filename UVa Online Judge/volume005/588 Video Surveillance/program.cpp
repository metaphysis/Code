...
polygon halfPlaneIntersection(line *edgeLine, int nLine) {
    ...
    for (int i = 2; i < nLine; i++) {
        if (paralleLine(dequeLine[top], dequeLine[top - 1]) || paralleLine(dequeLine[bottom], dequeLine[bottom + 1])) return pg;
        while (bottom < top && cw(edgeLine[i].a, edgeLine[i].b, getIntersection(dequeLine[top], dequeLine[top - 1]))) top--;
        while (bottom < top && cw(edgeLine[i].a, edgeLine[i].b, getIntersection(dequeLine[bottom], dequeLine[bottom + 1]))) bottom++;
        dequeLine[++top] = edgeLine[i];
    }
    while (bottom < top && cw(dequeLine[bottom].a, dequeLine[bottom].b, getIntersection(dequeLine[top], dequeLine[top - 1]))) top--;
    while (bottom < top && cw(dequeLine[top].a, dequeLine[top].b, getIntersection(dequeLine[bottom], dequeLine[bottom + 1]))) bottom++;
    if (top <= (bottom + 1)) return pg;
    // 求相邻两条凸包边的交点获取顶点坐标
    for (int i = bottom; i < top; i++) pg.vertex[pg.numberOfVertex++] = getIntersection(dequeLine[i], dequeLine[i + 1]);
    // 首尾两条直线的交点也是顶点
    if (bottom < (top + 1)) pg.vertex[pg.numberOfVertex++] = getIntersection(dequeLine[bottom], dequeLine[top]);
    return pg;
}
...
int main() {
    ...
    while (cin >> n, n) {
        ...
        cout << "Floor #" << ++cases << '\n';
        if (pg.numberOfVertex > 0) cout << "Surveillance is possible.\n\n";
