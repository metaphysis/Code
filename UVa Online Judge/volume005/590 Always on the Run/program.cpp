    ...
    while (cin >> n >> k, n) {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            for (int j = 1; j <= n; j++) {
                ...
                for (int l = 0; l < count; l++) cin >> lines[l];
                schedule[i].push_back(lines);
            }
        }
        memset(cost, 0, sizeof(cost));
        for (int i = 1; i <= n; i++) cost[1][i] = schedule[1][i][0];
        for (int i = 2; i <= k; i++) {
            for (int j = 1; j <= n; j++) {
                for (int l = 1; l <= n; l++) {
                    if (l == j) continue;
                    ...
                    if (cost[i][l] == 0) cost[i][l] = cost[i - 1][j] + fee;
                    else cost[i][l] = min(cost[i][l], cost[i - 1][j] + fee);
                }
            }
        }
        cout << "Scenario #" << ++cases << '\n';
