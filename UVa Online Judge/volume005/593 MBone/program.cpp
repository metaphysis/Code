...
int main() {
    ...
    while (cin >> islands, islands) {
        ...
        for (int c = 1; c <= islands; c++) {
            ...
            if (indexer.find(source) == indexer.end()) indexer[source] = routerId++;
            for (int d = 1; d <= datalines; d++) {
                cin >> kinds;
                if (kinds == 'T') {
                    cin >> cost >> dest;
                    if (indexer.find(dest) == indexer.end()) indexer[dest] = routerId++;
        ...
        for (int c = 1; c <= activities; c++) {
            ...
            if (kinds == 'J') {
                ...
                for (auto host : groups[groupId]) {
                    ...
                    if (ttl >= dist[from][to]) packetReceived[host].push_back(make_pair(packetId, ttl - dist[from][to]));
                }
            }
        }
        cout << "Network #" << ++cases << '\n';
        for (auto packet : packetReceived) {
            ...
            for (auto data : packet.second) cout << packet.first << ' ' << data.first << ' ' << data.second << '\n';
        }
        cout << '\n';
    }
    return 0;
