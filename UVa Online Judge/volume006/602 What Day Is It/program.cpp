...
int getDay(date date1) {
    int days = 0;
    for (int i = 1; i < date1.year; i++) days += (isLeapYear(i, date1.isOldFormat) ? 366 : 365);
    for (int i = 1; i < date1.month; i++) days += daysInMonth[isLeapYear(date1.year, date1.isOldFormat)][i - 1];
...
int main(int argc, char *argv[]) {
    ...
    while (cin >> date1.month >> date1.dayOfMonth >> date1.year) {
        ...
        // 闰年判断（用于2月）
        bool leap = false;
        if (date1.year > 1752) leap = date1.year % 400 == 0 || (date1.year % 4 == 0 && date1.year % 100 != 0);
        else leap = date1.year % 4 == 0;
        if (!leap && date1.month == 2 && date1.dayOfMonth > 28) valid = false;
        if (!valid) {
