...
int main(int argc, char *argv[]) {
    ...
    for (int c = 1; c <= cases; c++) {
        ...
        while (getline(cin, line), line.length() > 0) {
            ...
            for (int i = 0; i < wait.size(); i++) wait[i].second = (wait[i].second + advanced) % 20;
        }
        for (int i = 0; i < autos.size(); i++) {
            cout << "Original position " << autos[i] << ' ';
            if (arranged.find(autos[i]) != arranged.end()) cout << "parked in " << arranged[autos[i]] + 1 << '\n';
            else cout << "did not park\n";
        }
    }
    return 0;
}
