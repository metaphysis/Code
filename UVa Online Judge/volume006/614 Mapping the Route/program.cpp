...
bool go(int i, int j, int d) {
    if (d == WEST && (j == 1 || maze[i][j - 1] == 1 || maze[i][j - 1] == 3)) return false;
    if (d == NORTH && (i == 1 || maze[i - 1][j] == 2 || maze[i - 1][j] == 3)) return false;
    if (d == EAST && (j == columns || maze[i][j] == 1 || maze[i][j] == 3)) return false;   
    if (d == SOUTH && (i == rows || maze[i][j] == 2 || maze[i][j] == 3)) return false;
    return true;
}
...
int main() {
    ...
    while (cin >> rows >> columns >> starti >> startj >> endi >> endj) {
        ...
        if (dfs(starti, startj)) {
            ...
            for (int i = 0; i < paths.size(); i++) visited[paths[i].first][paths[i].second] = i + 1;
        }
        for (int i = 1; i <= columns; i++) cout << "+---";
        cout << "+\n";
        for (int i = 1; i <= rows; i++) {
            if (i > 1) {
                for (int j = 1; j <= columns; j++) {
                    cout << '+';
                    if (maze[i - 1][j] == 2 || maze[i - 1][j] == 3) cout << "---";
                    else cout << "   ";
                }
                cout << "+\n";
            }
            for (int j = 1; j <= columns; j++) {
                ...
                else cout << setw(3) << right << visited[i][j];
                if (maze[i][j] == 1 || maze[i][j] == 3 || j == columns) cout << '|';
                else cout << ' ';
            }
            cout << '\n';
        }
