...
bool segmentsIntersect(segment ray, segment side) {
    ...
    if ((d1 * d2 > 0) || (d3 * d4 > 0)) return false;   
	  // 共线，不论线段是否重合，均规定为不相交。
    if ((fabs(d1) <= EPSILON && fabs(d2) <= EPSILON) || (fabs(d3) <= EPSILON && fabs(d4) <= EPSILON)) return false;
	  // 相交于顶点，判断是否在射线上方。
    if (fabs(d1) <= EPSILON || fabs(d2) <= EPSILON || fabs(d3) <= EPSILON || fabs(d4) <= EPSILON) return isSideAboveRay(ray, side);
    return false;
}
