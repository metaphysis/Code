...
int main() {
    ...
    while (cin >> n, n) {
        ...
        for (int i = 0; i < sheet_number; i++) {
            ...
            if (sheets[i].front_left == 0) cout << "Blank";
            else cout << sheets[i].front_left;
            cout << ", " << sheets[i].front_right << '\n';
            if (sheets[i].back_left == 0 && sheets[i].back_right == 0) continue;
            else {
                cout << "Sheet " << i + 1 << ", back : ";
                cout << sheets[i].back_left << ", ";
                if (sheets[i].back_right == 0) cout << "Blank\n";
                else cout << sheets[i].back_right << '\n';
            }
        }
    }
    return 0;
