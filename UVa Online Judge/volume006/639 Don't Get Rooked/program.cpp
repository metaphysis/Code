...
bool floodFill(int position) {
    ...
    for (int next_i = i + 1; next_i < n; next_i++)
        if (backup[next_i * n + j] == 'r') return false;
        else if (backup[next_i * n + j] == 'X') break;
    for (int next_i = i - 1; next_i >= 0; next_i--)
        if (backup[next_i * n + j] == 'r') return false;
        else if (backup[next_i * n + j] == 'X') break;
    for (int next_j = j + 1; next_j < n; next_j++)
        if (backup[i * n + next_j] == 'r') return false;
        else if (backup[i * n + next_j] == 'X') break;
...
void backtrack(int position, int rooks) {
    ...
    for (int i = position; i < n * n; i++) {
        if (grid[i] == '.') {
            grid[i] = 'r';
            if (check()) backtrack(position + 1, rooks + 1);
            grid[i] = '.';
        }
    }
}
