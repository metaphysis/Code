...
int main() {
    ...
    for (int cs = 1; cs <= M; cs++) {
        ...
        for (int i = 0; i < N; i++) V[G[i].ti][G[i].si] += G[i].pi;
        dp[0][0] = 0;
        for (int i = 1; i <= T; i++)
            for (int j = 0; j <= K; j++)
                if (~dp[i - 1][j]) {
                    dp[i][j] = max(dp[i][j], dp[i - 1][j] + V[i][j]);
                    if (j) dp[i][j - 1] = max(dp[i][j - 1], dp[i - 1][j] + V[i][j - 1]);
                    if (j < K) dp[i][j + 1] = max(dp[i][j + 1], dp[i - 1][j] + V[i][j + 1]);
                }
        if (cs > 1) cout << '\n';
        int maximum = 0;
        for (int i = 0; i <= K; i++)
