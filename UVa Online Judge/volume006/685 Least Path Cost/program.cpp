...
void dfs(int i, int length, int previous, int cost) {
    if (minCost != -1 && length >= 2 && cost > minCost) return;
    if (previous != -1 && length >= 2 && edges[i].size() == 1) {
        ...
        for (auto e : edges[i]) {
            ...
            visited[i] = 1;
            if (previous == -1) dfs(e.end, length + 1, e.height, cost + D);
            else dfs(e.end, length + 1, e.height, cost + D + abs(e.height - previous));
            visited[i] = 0;
        }
    }
}
