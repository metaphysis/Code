...
void rotate(string &config, int direction) {
	...
	switch (direction) {
		...
		case LEFT_CLOCKWISE:
			...
			for (int i = HALF_WHEEL - 1; i >= 2; i--) config[i] = config[i - 2];
			config[1] = middle[2];
			config[0] = middle[1];
			break;
			// 右侧滑块顺时针旋转。
		case RIGHT_CLOCKWISE:
			...
			for (int i = HALF_WHEEL; i <= (HALF_WHEEL * 2 - 3); i++) config[i] = config[i + 2];
			config[HALF_WHEEL * 2 - 2] = middle[0];
		...
		case LEFT_COUNTERCLOCKWISE:
			...
			for (int i = 0; i <= HALF_WHEEL - 3; i++) config[i] = config[i + 2];
		...
		case RIGHT_COUNTERCLOCKWISE:
			...
			config[HALF_WHEEL * 2 + 2] = middle[0];
			for (int i = 2 * HALF_WHEEL - 1; i >= HALF_WHEEL + 2; i--) config[i] = config[i - 2];
...
void backward_search(string config) {
	...
	while (!open.empty()) {
		...
		if (copy.sequences.length() >= BACKWARD_DEPTH) continue;
		for (int i = LEFT_CLOCKWISE; i <= RIGHT_COUNTERCLOCKWISE; i++) {
			...
			if (copy.sequences.length() > 0) {
				...
				int last_rotate = reversed[copy.sequences[0] - '0' - 1];
			...
			if (cache.find(t) == cache.end()) {
				...
				successor.sequences = (char)('0' + reversed[i - 1]) + copy.sequences;
				open.push(successor);
				cache.insert({t, successor.sequences});
			}
		}
	}
}
...
bool forward_search(string config) {
	...
	while (!open.empty()) {
		...
		// 搜索深度为 9。
		if (copy.sequences.length() >= (BACKWARD_DEPTH + 1)) continue;
		for (int i = LEFT_CLOCKWISE; i <= RIGHT_COUNTERCLOCKWISE; i++) {
			// 若前后两步构成互补状态则跳过。
			if (copy.sequences.length() > 0) {
...
int main() {
	...
	while (cases--) {
		...
		for (int i = 0; i < NWHEEL; i++) {
			cin >> c;
			if (c == 10) config.append(1, 'T');
			else config.append(1, c + '0');
		}
		// 调整表示形式。
		...
		// 进行正向搜索查找。
		if (!forward_search(config)) cout << "NO SOLUTION WAS FOUND IN 16 STEPS" << endl;
	}
	return 0;
}
