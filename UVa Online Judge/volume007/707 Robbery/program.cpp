...
void backward_dfs(int time, int y, int x) {
	...
	for (int i = 0; i < 5; i++) {
		...
		if (1 <= tmpY && tmpY <= height && 1 <= tmpX && tmpX <= width)
			...
			else if (grid[time - 1][tmpY][tmpX] == REACHABLE) grid[time][y][x] = REACHABLE;
	}
}
...
void forward_dfs(int time, int y, int x) {
	...
	for (int i = 0; i < 5; i++) {
		...
		if (1 <= tmpY && tmpY <= height && 1 <= tmpX && tmpX <= width)
			...
			else if (grid[time + 1][tmpY][tmpX] == I_AM_HERE) grid[time][y][x] = I_AM_HERE;
	}
}
