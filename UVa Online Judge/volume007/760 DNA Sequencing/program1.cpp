...
int main() {
    ...
    while (cin >> s1 >> s2) {
        ...
        for (int i = 1; i <= n; i++)
            if (height[i] >= longest) {
                if ((separator < sa[i - 1] && separator > sa[i]) || (separator > sa[i - 1] && separator < sa[i])) {
                    if (height[i] > longest) common.clear();
                    longest = height[i];
                    if (separator > sa[i]) common.insert(s1.substr(sa[i], longest));
                    else common.insert(s1.substr(sa[i - 1], longest));
                }
            }
        if (common.size() > 0) {
