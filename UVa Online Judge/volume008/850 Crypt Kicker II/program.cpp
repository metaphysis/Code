...
void printPlain(string cipher) {
    for (int m = 0; m < encrypted.size(); m++) {
        for (int n = 0; n < encrypted[m].size(); n++) {
            ...
            for (int j = 0; j < tmp.length(); j++) cout << cipher[tmp[j] - 'a'];
            if (n < encrypted[m].size() - 1) cout << " ";
        }
        cout << endl;
    }
}
...
int main() {
    ...
    while (iss >> word) plain.push_back(word);
    cin >> cases;
    cin.ignore();
    getline(cin, line);
    while (cases--) {
