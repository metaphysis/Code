...
// UVa Run Time: 0.020s
//
// https://github.com/morris821028/UVa/blob/master/volume012/1254%20-%20Top%2010.cpp
...
void queryACautomaiton(const char *str, Node * root, int number)
{
    static Node *p, *q;
    static int i, idx;
    p = root;
    for (i = 0; str[i]; i++)
    {
        ...
        {
            int bflag = 0;
            for (vector < int >::iterator it = q->label.begin(); it != q->label.end(); it++)
            {
                if (Top10[*it].size() == 10)
                {
                    bflag = 1;
...
void sortTrie(Node * root, Node * qroot)
{
    ...
    while (!Q.empty())
    {
        ...
        for (vector < int >::iterator it = tn->label.begin(); it != tn->label.end(); it++)
        {
            queryACautomaiton(word[*it].c_str(), qroot, *it);
            //cout << word[*it] << endl;
        }
    }
}
