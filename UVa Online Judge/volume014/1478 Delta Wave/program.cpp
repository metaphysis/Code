...
public:
    ...
    string last100Digits() const
    {
        ...
        while (text.length() > 100) text.erase(text.begin());
        while (text.front() == '0') text.erase(text.begin());
        return text;
    }
...
int main(int argc, char *argv[])
{
    ...
    while (cin >> n)
    {
        BigInteger waves = ZERO, tmp = ONE;
        
        waves = waves + tmp;
        for (int i = 1, up = n / 2; i <= up; i++)
        {
            tmp = tmp * ((n - 2 * i + 2) * (n - 2 * i + 1)) / (i * (i + 1));
            waves = waves + tmp;
        }
        
        cout << waves.last100Digits() << '\n';
    }
