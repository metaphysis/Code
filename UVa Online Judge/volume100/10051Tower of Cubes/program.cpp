...
//
// [问题描述]
// 给你 N 个质量各异的彩色立方体。每个立方体都不是单色的 —— 事实上，它们的每一面都被
// 涂上了不同的颜色。你的工作是要用这些立方体建一座最高的塔，使得（1）每个立方体上面的
// 那个立方体都比它轻，（2）每个立方体（除了塔底的立方体）底面的颜色必须和它下面那个
// 立方体的顶面颜色相同。
//
// [输入]
// 输入包含若干组数据。每组数据的第一行为一个整数 N（1 <= N <= 500），表示立方体的
// 数量。接下来的 N 行中，第 i 行描述了第 i 个立方体。该描述中的六个数分别表示这个
// 立方体前、后、左、右、上、下这几个面的颜色，颜色都用 1 ～ 100 之间的整数代表。假设
// 输入立方体的质量是递增的，即第 1 个是最轻的而第 N 个是最重的。
//
// 当 N = 0 是输入结束。
//
// [输出]
// 对于每组数据，按照样例格式现在单独的一行输出数据的序号，然后在下一行输出塔的最大
// 高度。接下来按从上到下的顺序描述塔的每一层。每行描述一个立方体：先给出它的输入序号，
// 然后是空格和一个表示着色方式的字符串（front，back，left，right，top 或者 bottom），
// 分别表示该立方体在塔中的顶面是输入中的那个面。如果有多种解，任意输出一种即可。
//
// 相邻两组数据的输出之间用一个空行隔开。
//
// [样例输入]
...
//
// [解题方法]
// 每个立方体的一面和它对应的一面构成了一条边，立方体之间又构成一些边，然后需要在这些
// 立方体之间寻找一条最长路径，这就是问题所求。将第一例测试数据转换一下可以看得更明白。
//
// 2 1 2 2 2 1
// | | | | | |  (立方体 1）
// 1 2 2 2 1 2
...
// 3 2 1 1 1 1
//
// 由上可知可将其建模成有向无环图，然后其最长路径即为所求。由于本题的特点，与 UVa 10029 
// 类似，也可以利用求 LIS（Longest Increasing Subsequence） 的算法予以解决，或者
// 直接使用动态规划来解决，这样可以更方便。为了简便，本题使用动态规划解决。由于立方体
// 的颜色总数最多为 100 种，则可以设立一个数组 tower，其中的元素 tower[i] 表示底面
// 颜色为 i 的塔的最大高度。对于读入的相对应的面对，如 front 和 back 组成一个面对，
