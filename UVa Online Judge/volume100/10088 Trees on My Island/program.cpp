...
// 利用有向面积计算多边形的面积。
llint calArea(polygon &contour)
{
	llint total = 0.0;

	for (llint i = 0; i < contour.n; i++)
	{
		llint j = (i + 1) % contour.n;
...
// 计数多边形边界上的格点。
llint boundaryPolints(polygon &contour)
{
	...
	for (llint i = 0; i < contour.n; i++)
	{
		...
		{
			// 求横坐标和纵坐标的差的绝对值。
			llint xDiff = abs(contour.p[j].x - contour.p[i].x);
			llint yDiff = abs(contour.p[j].y - contour.p[i].y);

			// 当 x 变化 xDiff 个单位时，y 变化 yDiff 个单位，只要找到这样的
			// 的点：点的横坐标在（min（p[i].x，p[j].x），max（p[i].x，p[j].x））
...
int main (int argc, char const* argv[])
{
	...
	while (cin >> vertexNumber, vertexNumber)
	{
		...
		for (llint i = 0; i < vertexNumber; i++)
			cin >> contour.p[i].x >> contour.p[i].y;
		
		// 统计该多边形的边界上的格点数。
		llint vertexBoundary = boundaryPolints(contour);

		// 由多边形面积计算公式计算多边形面积，注意计算结果是面积实际值乘以 2，这样避免了小数。
		llint area = calArea(contour);
