...
//
// [问题描述]
// 某城市中有一些消防队，居民抱怨说一些房子离最近的消防站都很远，所以必须建一个新的消
// 防站。你要选择这个新消防站的位置，让它离最缺少服务的居民区最近。
//
// 这个城市最多有 500 个路口，被长度不同的道路连接。汇集在同一路口的道路数不会超过 20。
// 房子和消防站的位置都在路口上。假定在每个路口至少有一栋房子，且在同一路口可以有多个
// 消防站。
//
// [输入]
// 输入第一行只有一个整数，代表测试数据的数量，接下来有一个空行。相邻两组数据之间也有
// 一个空行。每组数据的第一行有两个整数：现有的消防站数量 f（f <= 100） 和路口的数量
//  i（i <= 500）。路口用 1 ～ i 的连续整数来表示。接下来的 f 行，每一行有一个数，表
// 示该消防站所在路口的编号。后面的若干行，每行有 3 个正整数：一个路口的编号，另一个
// 路口的编号，和连接这两个路口道路的长度。所有的道路都是双向的（至少消防车可以从两边
// 通过），并且任何两个路口之间的都是可达的。
//
// [输出]
// 对于每一组数据，输出新消防站所在的路口编号（使得任意路口到最近消防站的最大距离最
// 小）。如果有多个路口满足条件，输出编号最小的一个。相邻两组数据的输出被一个空行隔开。
//
// [样例输入]
// 1
//
...
//
// [解题方法]
// 如何求出路口到最近消防站的距离？回忆学习过的图算法，可以通过求每对结点之间的最短路
// 长度来得到，即 Floyd 算法。通过 Floyd 算法，可以方便的从图的边权矩阵直接算出距离
// 矩阵，然后通过比较路口到各个消防站的距离得到至最近消防站的距离，然后再比较各个路口
// 至最近消防站的距离得到路口离消防站的最大距离，由于新消防站只能位于所有路口中的一个，
