...
inline void qPush(int x) { q[tail++] = x; }
inline int qPop() { return q[head++]; }
...
int main() {
    ...
    for (int i = 0; i < M; i++) {
        ...
        if (mv == 0) {
            for (int j = 0; j < 8 && !win; j++)
                if (!ways[k0][j].empty()) {
                    int nxt = ways[k0][j][0];
                    if (nxt == k1) { win = true; break; }
                    if (nxt != q0) ++res;
                }
            if (q0 != k0)
                for (int j = 0; j < 8 && !win; j++)
                    for (auto s : ways[q0][j]) {
                        if (s == k1) { win = true; break; }
                        if (s == k0) break;
                        ++res;
                        if (s == r1) break;
                    }
            ...
            for (int j = 0; j < 8 && !win; j++)
                if (!ways[k1][j].empty()) {
                    int nxt = ways[k1][j][0];
                    if (nxt == k0) { win = true; break; }
                    if (nxt != r1) ++res;
                }
            if (r1 != k1)
                for (int j = 0; j < 4 && !win; j++)
                    for (auto s : ways[r1][j]) {
                        if (s == k0) { win = true; break; }
                        if (s == k1) break;
                        ++res;
                        if (s == q0) break;
                    }
        ...
        if (mv == 0) {
            for (int j = 0; j < 8 && !canMove; j++)
                if (!ways[k0][j].empty()) {
                    int nxt = ways[k0][j][0];
                    if (nxt != k1 && nxt != q0) {
                        int id = fromStatus(nxt, q0, k1, r1 == nxt ? k1 : r1, 1);
                        canMove |= !isCheckStatus[id];
                    }
                }
            if (q0 != k0)
                for (int j = 0; j < 8 && !canMove; j++)
                    for (auto s : ways[q0][j]) {
                        if (s == k1 || s == k0) break;
                        int id = fromStatus(k0, s, k1, r1 == s ? k1 : r1, 1);
                        canMove |= !isCheckStatus[id];
                        if (s == r1) break;
                    }
        } else {
            for (int j = 0; j < 8 && !canMove; j++)
                if (!ways[k1][j].empty()) {
                    int nxt = ways[k1][j][0];
                    if (nxt != k0 && nxt != r1) {
                        int id = fromStatus(k0, q0 == nxt ? k0 : q0, nxt, r1, 0);
                        canMove |= !isCheckStatus[id];
                    }
                }
            if (r1 != k1)
                for (int j = 0; j < 4 && !canMove; j++)
                    for (auto s : ways[r1][j]) {
                        if (s == k0 || s == k1) break;
                        int id = fromStatus(k0, q0 == s ? k0 : q0, k1, s, 0);
                        canMove |= !isCheckStatus[id];
                        if (s == q0) break;
                    }
        }
            ...
            for (int j = 0; j < 8; j++)
                if (!ways[k0][j].empty()) {
                    ...
                    if (nxt != q0 && nxt != k1 && nxt != r1) {
                        ...
                        if (rookDead) tryTransfer(fromStatus(nxt, queenDead ? nxt : q0, k1, k0, 0), st[S]);
                    }
                }
            if (!queenDead)
                for (int j = 0; j < 8; j++)
                    for (auto s : ways[q0][j]) {
                        ...
                        if (rookDead) tryTransfer(fromStatus(k0, s, k1, q0, 0), st[S]);
                    }
        } else {
            for (int j = 0; j < 8; j++)
                if (!ways[k1][j].empty()) {
                    ...
                    if (nxt != q0 && nxt != k0 && nxt != r1) {
                        ...
                        if (queenDead) tryTransfer(fromStatus(k0, k1, nxt, rookDead ? nxt : r1, 1), st[S]);
                    }
                }
            if (!rookDead)
                for (int j = 0; j < 4; j++)
                    for (auto s : ways[r1][j]) {
                        ...
                        if (queenDead) tryTransfer(fromStatus(k0, r1, k1, s, 1), st[S]);
                    }
        }
    }
    for (int i = 0; i < M; i++)
    ...
    while (t--) {
        ...
        if (st[id].sig == 0) puts("Draw");
        else printf("%s mates in %d\n", ((st[id].sig == 1) ^ (first == 'B')) ? "White" : "Black", st[id].num / 2);
        if (st[id].num == 1) {
            printf("\n\n");
            continue;
        }
        ...
        if (mv == 0) {
            for (int j = 0; j < 8; j++)
                if (!ways[k0][j].empty()) {
                    ...
                    if (nxt != k1 && nxt != q0) {
                        int id = fromStatus(nxt, q0, k1, r1 == nxt ? k1 : r1, 1);
                        if (st[id] == expect) answers.push_back(genMove('K', nxt, r1 == nxt, isCheckStatus[id ^ 1], mateIf));
                    }
                }
            for (int j = 0; j < 8; j++)
                for (auto s : ways[q0][j]) {
                    if (s == k1 || s == k0) break;
                    int id = fromStatus(k0, s, k1, r1 == s ? k1 : r1, 1);
                    if (st[id] == expect) answers.push_back(genMove('Q', s, r1 == s, isCheckStatus[id ^ 1], mateIf));
                    if (s == r1) break;
                }
        } else {
            for (int j = 0; j < 8; j++)
                if (!ways[k1][j].empty()) {
                    ...
                    if (nxt != k0 && nxt != r1) {
                        int id = fromStatus(k0, q0 == nxt ? k0 : q0, nxt, r1, 0);
                        if (st[id] == expect) answers.push_back(genMove('K', nxt, q0 == nxt, isCheckStatus[id ^ 1], mateIf));
                    }
                }
            for (int j = 0; j < 4; j++)
                for (auto s : ways[r1][j]) {
                    if (s == k0 || s == k1) break;
                    int id = fromStatus(k0, q0 == s ? k0 : q0, k1, s, 0);
                    if (st[id] == expect) answers.push_back(genMove('R', s, q0 == s, isCheckStatus[id ^ 1], mateIf));
                    if (s == q0) break;
                }
        }
