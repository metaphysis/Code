...
int main(int argc, char *argv[])
{
    ...
    for (int cs = 1; cs <= cases; cs++)
    {
        cin >> t1 >> t2 >> s1 >> s2 >> w;
        line sides[6] = {
            pointToLine(point(t1, s1), point(t2, s1)),
            pointToLine(point(t2, s1), point(t2, s2)),
            pointToLine(point(t2, s2), point(t1, s2)),
            pointToLine(point(t1, s2), point(t1, s1)),
            pointToLine(point(0, -w), point(w, 0)),
            pointToLine(point(0, w), point(-w, 0))
        };
        polygon pg = halfPlaneIntersection(sides, 6);
        cout << "Case #" << cs << ": ";
        cout << fixed << setprecision(8) << getArea(pg) / ((t2 - t1) * (s2 - s1)) << '\n';
    }
